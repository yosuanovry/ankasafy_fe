...
const userRegister = (e) => {
  ...
  axios.post(url + `auth/register/customer`, data)
  .then((res) => {
    console.log(res)
  }).catch((err) => {
    console.log(err)
          ...
          <form onSubmit={userRegister}>
            ...
            <label className="inline-flex items-center my-2 ms-2">
              <input
                required
                type="checkbox"
                className="rounded border-blue-500 text-indigo-600 shadow-sm focus:border-blue-500 focus:ring focus:ring-offset-0 focus:ring-indigo-200 focus:ring-opacity-50"
              />
              <span className="ml-2 text-xxs sm:text-md">
                Accept terms and condition
              </span>
            </label>
            <span className="flex items-center justify-center mt-8 lg:mt-16 text-xxs lg:text-md">
            ...
            <span className="flex justify-center items-center mt-4 rounded-lg text-sm py-1 md:py-4 font-semibold border-blue-500 border-[1px] sm:text-base md:text-lg" style={{ backgroundColor: "white", color: "#2395FF" }}>
              <button type="submit" style={{width:455}}>Sign In</button>
            </span>
          </form>
        </div>
      </div>
